...
if [ "${CONSOLE}" ]; then
    docker run --env BITCOIN_DATA=/media/block_data -v ${BLOCKDATA_DIR}:/media/block_data ${PORTS} --name=${NETWORKNAME} ${ENTRY} --rm -it uphold/bitcoin-abc
else
    docker run --env BITCOIN_DATA=/media/block_data -v ${BLOCKDATA_DIR}:/media/block_data ${PORTS} --name=${NETWORKNAME} --rm -it uphold/bitcoin-abc \
	-printtoconsole \
